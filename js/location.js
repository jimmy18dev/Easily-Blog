    ...
    $('#findLocation').on('input',function(event) {
        ...
        $.ajax({
            ...
            $.each(data.dataset,function(k,v){
                ...
                html += '<li class="location-items" data-district="'+v.district_id+'" data-amphur="'+v.amphur_id+'" data-province="'+v.province_id+'">'+(v.district_name?'ต.'+v.district_name+' ':'')+(v.amphur_name?'อ.'+v.amphur_name+' ':'')+(v.province_name?'จ.'+v.province_name:'')+'<i class="fal fa-check"></i></li>';
